...
/**
 * Processes M3U8 content for CDN delivery
 * Rewrites relative paths to point to CDN endpoints
 * 
 * @param content The M3U8 content as string
 * @param cdnBaseUrl The base CDN URL (e.g., "https://example.com/cdn/bucket/")
 * @param currentPath The current M3U8 file path in S3
 * @returns Processed M3U8 content with rewritten paths
 */
export function processM3U8ContentForCDN(
  content: string,
  cdnBaseUrl: string,
  currentPath: string
): string {
  try {
    // Get the directory path of the current M3U8 file
    ...
    const processedLines = lines.map(line => {
      ...
      if (trimmedLine.startsWith('#')) {
        ...
        if (line.includes('URI="')) {
          ...
          if (uriMatch && uriMatch[1]) {
            ...
            let cdnPath: string;
            if (originalUri.startsWith('http://') || originalUri.startsWith('https://')) {
              // Absolute URL - extract path or use as-is (this shouldn't happen in CDN context)
              cdnPath = originalUri;
            } else if (originalUri.startsWith('/')) {
              // Absolute path from root
              cdnPath = `${cdnBaseUrl}${originalUri.substring(1)}`;
            } else {
              // Relative path - resolve against base directory
              cdnPath = `${cdnBaseUrl}${baseDir}${originalUri}`;
            }
            
            // Replace the URI in the original line
            return line.replace(/URI="([^"]+)"/, `URI="${cdnPath}"`);
          }
        }
      ...
      // Handle non-comment lines that might be segment paths
      if (trimmedLine.length > 0) {
        // Ignore if it already looks like a CDN URL
        if (trimmedLine.startsWith('http://') || trimmedLine.startsWith('https://')) {
          return line;
        }
        ...
        if (trimmedLine.startsWith('/')) {
          // Absolute path from root
          cdnPath = `${cdnBaseUrl}${trimmedLine.substring(1)}`;
        } else {
          // Relative path - resolve against base directory
          cdnPath = `${cdnBaseUrl}${baseDir}${trimmedLine}`;
        }
    ...
    console.error('Error processing M3U8 content for CDN:', error);
    // Return original content on error to avoid breaking playback
    return content;
  }
}
